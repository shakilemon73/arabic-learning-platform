...
export const useSupabaseAuth = () => {
  ...
  const ensureUserProfile = async (authUser: SupabaseUser) => {
    // Prevent infinite loops by checking if we already have a profile
    if (userProfile) {
      return;
    }
    ...
    try {
      ...
      // Only try to create profile if user doesn't exist (PGRST116)
      ...
      // Silently continue if profile operations fail
    }
  };
