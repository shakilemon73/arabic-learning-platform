-- WorldClass Video SDK Database Schema - CORRECTED VERSION
...
CREATE INDEX IF NOT EXISTS idx_whiteboard_room_time ON whiteboard_actions(room_id, created_at);
CREATE INDEX IF NOT EXISTS idx_moderation_room_time ON moderation_actions(room_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_permissions_room_user ON participant_permissions(room_id, user_id);
CREATE INDEX IF NOT EXISTS idx_quality_logs_room_time ON stream_quality_logs(room_id, created_at DESC);
...
-- Function to automatically end room when last participant leaves (FIXED)
CREATE OR REPLACE FUNCTION check_room_empty()
RETURNS TRIGGER AS $$
BEGIN
    -- Only proceed if this is a disconnection event
    IF OLD.is_connected = true AND NEW.is_connected = false THEN
        -- Check if room is now empty of connected participants
        IF NOT EXISTS (
            SELECT 1 FROM participants 
            WHERE participants.room_id = NEW.room_id 
            AND participants.is_connected = true
            AND participants.id != NEW.id  -- Exclude the current disconnecting participant
        ) THEN
...
EXCEPTION 
    WHEN others THEN NULL; -- Ignore if storage schema doesn't exist
END $$;

-- Storage policies (only if storage exists)
DO $$
BEGIN
    -- Chat files policies
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'storage' AND table_name = 'objects') THEN
        ...
        DROP POLICY IF EXISTS "Moderators can upload recordings" ON storage.objects;
        DROP POLICY IF EXISTS "Room participants can view recordings" ON storage.objects;
...
-- Uncomment the following line to add sample room data
-- INSERT INTO rooms (name, description, is_public, max_participants) VALUES ('Arabic Learning Demo', 'Interactive Arabic language learning session', true, 50);
...
DO $$
BEGIN
    RAISE NOTICE 'WorldClass Video SDK database schema created successfully!';
    RAISE NOTICE 'Created tables: rooms, participants, chat_messages, chat_reactions, recordings, whiteboard_actions, moderation_actions, participant_permissions, room_settings, stream_quality_logs';
